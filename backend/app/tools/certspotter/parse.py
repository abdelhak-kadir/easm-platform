"""Parse CertSpotter CT search response into findings."""

from app.models import Severity


def parse(raw_data: dict) -> list[dict]:
    """Turn raw CertSpotter output into Finding-ready dicts.

    Produces a single ``discovered_assets`` finding with ``category: "hosts"``
    — same schema as Subfinder/theHarvester/Amass/MerkleMap, so the existing
